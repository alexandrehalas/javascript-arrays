...
const calculateBalance = function (movements) {
  return movements.reduce((accumulated, movement) => accumulated + movement, 0);
};

const displayBalance = function (account) {
  account.balance = calculateBalance(account.movements);
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  const dogAges = dogsJuliaCorrect.concat(dogsKate);

  dogAges.forEach(function (dogAge, i) {
    const message = `Dog number ${i + 1} is `;
    if (dogAge > 3) {
      console.log(message.concat(`an adult, and is ${dogAge} years old`));
    } else {
      console.log(message.concat(`is still a puppy 🐶`));
    }
  });
};
...
const calcAverageHumanAge = ages =>
  ages
    ...
    .reduce(
      (accumulator, age, index, array) => accumulator + age / array.length,
      0
    );
...
console.log(shallowDogs.sort((a, b) => a.recommendedFood - b.recommendedFood));
console.log(dogs);
